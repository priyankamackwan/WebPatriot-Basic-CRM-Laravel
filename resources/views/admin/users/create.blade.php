    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('phone_otp') ? 'is-invalid' : '' }}" type="text" name="phone_otp" id="phone_otp" value="{{ old('phone_otp', '') }}">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\User::OTP_VERIFIED_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('otp_verified') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="otp_verified_{{ $key }}" name="otp_verified" value="{{ $key }}" {{ old('otp_verified', '') === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\User::PROVIDER_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('provider') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="provider_{{ $key }}" name="provider" value="{{ $key }}" {{ old('provider', '') === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                <label for="provider_response">{{ trans('cruds.user.fields.provider_response') }}</label>
                <textarea class="form-control {{ $errors->has('provider_response') ? 'is-invalid' : '' }}" name="provider_response" id="provider_response">{{ old('provider_response') }}</textarea>
                @if($errors->has('provider_response'))
                    <div class="invalid-feedback">
                        {{ $errors->first('provider_response') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.user.fields.provider_response_helper') }}</span>
            </div>
            <div class="form-group">
